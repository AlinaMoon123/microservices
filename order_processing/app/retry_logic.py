...
def retry_topic(retry_num:int):
    return f'inventory.retry.{retry_num}'


def handle_retry(event:dict, order_id:int, try_number:int, delay_delta:int):
    retry = event.get("retry", 0)

    if retry < try_number:
        delay=delay_delta*(retry+1)
        ...
        send_event('inventory.dlq', event, order_id)
